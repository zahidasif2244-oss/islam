...
export async function GET(req: Request, { params }: { params: Promise<{ book: string }> }) {
  ...
  try {
    ...
    if (words.length > 1) {
      const textConditions = words.map(w => `hl_ur.hadees LIKE ?`)
      whereClause = textConditions.join(' AND ')
      params = words.map(w => `%${w}%`)
    } else {
      whereClause = `CAST(h.hadees_number AS TEXT) LIKE ? OR hl_ur.hadees LIKE ? OR hl_en.hadees LIKE ? OR h.arabic LIKE ?`
