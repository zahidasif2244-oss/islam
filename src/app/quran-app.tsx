...
type InitialData = { surahNames: any[]; parahNames: any[]; translations: any[]; tafseerTypes: any[] }

export default function QuranApp({ initial }: { initial: InitialData }) {
  ...
  const [tarjmaList, setTarjmaList] = useState<any[]>(initial.translations)
  const [tafseerList, setTafseerList] = useState<any[]>(initial.tafseerTypes)
    ...
    <SettingsCtx.Provider value={ctxVal}>
      <div>
        ...
        <div className="p-3 sm:p-4 max-w-[1200px] mx-auto">
          {tab === 'quran' && <QuranTab key={homeKey} initialSurahNames={initial.surahNames} initialParahNames={initial.parahNames} />}
...
function QuranTab({ initialSurahNames, initialParahNames }: { initialSurahNames: any[]; initialParahNames: any[] }) {
  const [view, setView] = useState<QuranView>('surahs')
  const [surahNames, setSurahNames] = useState<any[]>(initialSurahNames)
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fadeIn">
      ...
      <div className="text-center mt-8 mb-4">
        <a href="/admin"
          className="text-xs text-[#bbb] hover:text-[#1a5c3a] transition-colors cursor-pointer no-underline">
          Admin
        </a>
      </div>
    </div>
